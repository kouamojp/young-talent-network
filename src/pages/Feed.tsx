...
const FeedPage: React.FC = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // Fetch posts, suggested talents, and orgs in parallel
      const [postsRes, talentsRes, orgsRes] = await Promise.all([
        ...
        supabase
          .from('profiles')
          .select('id, name, avatar_url, user_type, sport_type, country')
          .neq('id', user.id)
          .limit(6),
        supabase
          .from('organization_profiles')
          .select('id, company_name, logo_url, industry, headquarters')
          .limit(4),
      ]);
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        <Tabs defaultValue="feed" className="w-full">
          <TabsList className="w-full justify-start mb-6 bg-muted/50">
            <TabsTrigger value="feed">Mon Feed</TabsTrigger>
                ...
                {posts.length === 0 ? (
                  <Card className="border-dashed border-2">
                    <CardContent className="py-12 text-center">
                      ...
                      <h3 className="font-semibold text-lg mb-2">Aucun post pour le moment</h3>
                      <p className="text-muted-foreground text-sm mb-4">Soyez le premier à partager quelque chose !</p>
                    </CardContent>
                  </Card>
                ) : (
                  posts.map(post => (
              ...
              <div className="space-y-4">
                ...
                <Card>
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      ...
                      <Button variant="ghost" size="sm" className="text-xs" onClick={() => navigate('/talents-around-me')}>
                        Voir tout
                      </Button>
                    </div>
                  </CardHeader>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-1">
                            <Badge variant="secondary" className="text-[10px] px-1.5 py-0">
                              {getUserTypeLabel(talent.user_type)}
                            </Badge>
                            {talent.country && (
                              <span className="text-[10px] text-muted-foreground">{talent.country}</span>
                            )}
                          </div>
                        </div>
                ...
                <Card>
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      ...
                      <Button variant="ghost" size="sm" className="text-xs" onClick={() => navigate('/organizations')}>
                        Voir tout
                      </Button>
                    </div>
                  </CardHeader>
                    ...
                    {suggestedOrgs.map((org) => (
                      <div key={org.id} className="flex items-center gap-3 cursor-pointer hover:bg-muted/50 rounded-lg p-1.5 -mx-1.5 transition-colors" onClick={() => navigate(`/organization/${org.id}`)}>
                        <Avatar className="h-9 w-9">
                          ...
                          <AvatarFallback className="text-xs bg-teal-100 text-teal-700">{org.company_name[0]}</AvatarFallback>
                        </Avatar>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium truncate">{org.company_name}</p>
                          <p className="text-[11px] text-muted-foreground truncate">{org.industry || org.headquarters || 'Organisation'}</p>
                        </div>
                ...
                <Card className="bg-gradient-to-br from-primary/5 to-primary/10 border-primary/20">
                  <CardContent className="p-4 space-y-2">
                    ...
                    <Button variant="outline" size="sm" className="w-full justify-start text-xs" onClick={() => navigate('/work')}>
                      <Briefcase className="h-3.5 w-3.5 mr-2" /> Chercher des opportunités
                    </Button>
                    <Button variant="outline" size="sm" className="w-full justify-start text-xs" onClick={() => navigate('/events')}>
                      <Bell className="h-3.5 w-3.5 mr-2" /> Événements à venir
                    </Button>
                    <Button variant="outline" size="sm" className="w-full justify-start text-xs" onClick={() => navigate('/test')}>
                      <Star className="h-3.5 w-3.5 mr-2" /> Tester mes aptitudes
                    </Button>
                  </CardContent>
                </Card>
              </div>
                ...
                <Card key={org.id} className="hover:shadow-md transition-shadow cursor-pointer" onClick={() => navigate(`/organization/${org.id}`)}>
                  <CardContent className="p-4 flex items-center gap-4">
                    <Avatar className="h-14 w-14">
                      <AvatarImage src={org.logo_url || ''} />
                      <AvatarFallback className="bg-teal-100 text-teal-700">{org.company_name[0]}</AvatarFallback>
                    </Avatar>
                    <div className="flex-1">
                      <h4 className="font-semibold">{org.company_name}</h4>
                      <p className="text-sm text-muted-foreground">{org.industry || 'Organisation'}</p>
                      {org.headquarters && (
                        <p className="text-xs text-muted-foreground">{org.headquarters}</p>
                      )}
                    </div>
