...
import { Users, Globe, Settings, Plus, MessageSquare, Clock } from 'lucide-react';
      ...
      <main className="container mx-auto px-4 py-12">
        <GlassMorphism className="p-6 mb-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <Button><Plus className="h-4 w-4 mr-2" />{t('communities.createCommunity')}</Button>
          </div>
                  ...
                  <GlassMorphism key={community.id} className="p-4 hover:shadow-md transition-shadow">
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <Button variant={community.isJoined ? "outline" : "default"} size="sm" className="mt-2">
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex gap-2 mt-2">
                          <Button size="sm" variant="outline"><MessageSquare className="h-3 w-3 mr-1" />{t('communities.view')}</Button>
                          <Button size="sm" variant="outline"><Settings className="h-3 w-3 mr-1" />{t('communities.settings')}</Button>
                        </div>
                      </div>
                    </div>
                  </GlassMorphism>
            ...
            {posts.map(post => (
              <GlassMorphism key={post.id} className="p-4">
                ...
                <div className="flex items-center justify-between text-sm">
                  <div className="flex gap-4">
                    <button className="flex items-center gap-1 text-gray-600 hover:text-primary"><span>👍</span> {post.likes}</button>
                    <button className="flex items-center gap-1 text-gray-600 hover:text-primary"><MessageSquare className="h-4 w-4" /> {post.comments}</button>
                  </div>
