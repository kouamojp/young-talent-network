...
import { Sidebar, SidebarHeader, SidebarGroup, SidebarGroupContent, SidebarGroupLabel, SidebarMenu, SidebarMenuItem, SidebarMenuButton } from '@/components/ui/sidebar';
import { mainNavigationItems, profileItems } from './sidebarData';
...
const SidebarMain: React.FC = () => {
  ...
  const renderMenuItems = (items: MenuSectionItem[]) => <SidebarMenu>
      {items.map(item => <SidebarMenuItem key={item.label}>
  ...
  return <Sidebar className="px-2 py-4 mx-0">
      <SidebarHeader>
        {/* Main Navigation */}
        <SidebarGroup>
